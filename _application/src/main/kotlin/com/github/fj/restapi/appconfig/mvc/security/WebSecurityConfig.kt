...
class WebSecurityConfig @Inject constructor(
    ...
    override fun configure(http: HttpSecurity) {
        http.cors().disable()
                ...
                .antMatchers("/${ApiPaths.HELLO}").permitAll()
                .antMatchers(HttpMethod.POST, "/${ApiPaths.ACCOUNT}").permitAll()
                .antMatchers(HttpMethod.PATCH, "/${ApiPaths.ACCOUNT}").hasAnyRole("USER")
                .antMatchers(HttpMethod.DELETE, "/${ApiPaths.ACCOUNT}").hasAnyRole("USER")
                // Restrict Spring actuator access except from localhost for security
                .antMatchers("/${ApiPaths.API_V1}/**").hasAnyRole("USER")
                .and()
                .formLogin()
