...
data class CreateAccountRequestDto @JvmOverloads constructor(
        ...
        @ApiModelProperty("A nickname, must be 2 to 16 letters.",
                example = "Good nickname", required = true)
        @JsonProperty
        val nickname: String = "",
        ...
        @ApiModelProperty("User's email address. Must be a valid email address or, " +
                "just leave it blank or null.", example = "username@company.com", required = false)
        @JsonProperty
        @ValidEmail
        val email: String? = "",

        @ApiModelProperty("Invitation host's user id token, which is 16 digit alphanumeric.",
    ...
    companion object : EmptyObject<CreateAccountRequestDto> {
        ...
        val VALIDATOR = object : NullsafeValidator<CreateAccountRequestDto> {
            ...
            override fun validateNonNull(target: CreateAccountRequestDto, e: Errors):
