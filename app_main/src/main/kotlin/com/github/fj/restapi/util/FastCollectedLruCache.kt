 ...
 *
 * Due to its GC friendly feature, many operations that based on size calculation, such as
 * `size()`, `entrySet()`, etc., could derive inconsistent results, therefore no such
 * map operations are offered.
 *
...
interface FastCollectedLruCache<K, V> {
    ...
    companion object {
        fun <K, V> create(maxCapacity: Int): FastCollectedLruCache<K, V> = Impl(maxCapacity)
    }

    private class Impl<K, V>(maxCapacity: Int) : FastCollectedLruCache<K, V> {
        private val hardCacheSize = Math.round(maxCapacity * 0.75f)
        private val softCacheSize = maxCapacity - hardCacheSize

        private val hardCache = object : LinkedHashMap<K, V>(hardCacheSize, 0.75f, true) {
        ...
        private val softCache = ConcurrentHashMap<K, SoftReference<V>>(softCacheSize)
